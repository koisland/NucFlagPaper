...
rule generate_filtered_calls:
    input:
        beds=lambda wc: expand(
            ...
            sm=[f"{wc.sm}_{dtype}" for dtype in ["hifi"]],
        ),
...
checkpoint merge_beds:
    ...
    output:
        calls=join(OUTPUT_DIR, "vg", "filtered_calls.bed"),
...
ASM_VAR_GRAPH_CONFIG = {
    ...
    "annotations": rules.merge_beds.output.calls,
...
# Find if more than one line.
# mkdir -p results/assembly/vg/graphs_nonempty && find results/assembly/vg/graphs -name "*.rgfa" -not -empty | \
# xargs -I {} bash -c 'if [ $(grep -Po "(?<=SN:Z:)(.*?)(?=\t)" {} | sort -u | wc -l) -gt 1 ]; then cp {} results/assembly/vg/graphs_nonempty; fi;'

# TODO: Call variation in each graph and overlap with NucFlag calls.
