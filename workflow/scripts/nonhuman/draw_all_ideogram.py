...
def draw_grouped_bar_w_calls_by_asm(
    ...
    for c in ax_grouped_calls.containers:
        ...
        ax_grouped_calls.bar_label(c, labels=labels, label_type="edge", fontsize=12)
    ...
    fig_grouped_calls.savefig(
        f"{output_prefix}_all_calls.png", dpi=300, bbox_inches="tight"
    )
    fig_grouped_calls.savefig(
        f"{output_prefix}_all_calls.pdf", dpi=300, bbox_inches="tight"
    )
